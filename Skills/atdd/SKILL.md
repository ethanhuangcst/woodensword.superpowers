...
name: atdd
description: >
  ATDD — write acceptance criteria and executable BDD specs (Given-When-Then / Gherkin) before
  implementation. Use for user stories, AC, scenario outlines, testability reviews, "acceptance tests",
  Gherkin, or BDD. Run before the **tdd** skill on new stories. One behavior per scenario; domain
  language; happy, error, and edge coverage.
---

# ATDD

Acceptance Test-Driven Development: write acceptance criteria and BDD-style specs **before** implementation.
Criteria should be testable, user-focused, and automatable. After AC is stable, hand off to the **tdd** skill for failing tests and code.

## Overview

Acceptance criteria define when a user story is complete. Well-written criteria provide:
