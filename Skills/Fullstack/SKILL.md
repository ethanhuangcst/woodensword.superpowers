...
description: >
  ...
  frontend+backend together. Apply woodensword rules: i18n for UI copy, Makefile dev/up/down on greenfield
  apps, common-test-strategy + tdd for logic, incremental-delivery per story.
license: MIT
---
...
## Project defaults (woodensword)

- **i18n**: no hard-coded user-facing strings (`i18n-support` rule)
- **Tests**: **tdd** + **common-test-strategy** for APIs and business logic
- **New repos**: root **Makefile** with `dev`, `up`, `down` (`makefile` rule)
- **Scope**: one user story to DoD before the next (`incremental-delivery`, `dod`)
